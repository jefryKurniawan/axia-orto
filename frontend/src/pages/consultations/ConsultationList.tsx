...
export default function ConsultationList() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Search + Filter Pills — outside card */}
          ...
          {isLoading ? (
            ...
            <div className="flex items-center gap-4 py-12 px-4">
              <div className="w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center flex-shrink-0">
                <Stethoscope className="w-6 h-6 text-blue-300 dark:text-blue-600" />
              </div>
              <div className="flex-1">
                <p className="text-sm font-medium text-slate-600 dark:text-slate-300">
                  {search || status ? 'Tidak ada konsultasi ditemukan' : 'Jadwal konsultasi kosong'}
                </p>
                {!search && !status && (
                  <p className="text-xs text-slate-400 dark:text-slate-500">Buat konsultasi baru untuk pasien Anda</p>
                )}
              </div>
              {!search && !status && (
                <Button size="sm" onClick={() => navigate('/consultations/create')}>+ Tambah</Button>
              )}
            </div>
          ) : (
            <>
